<x-admin-layout>
    <div class="max-w-3xl">
        <div class="mb-6">
            ...
            <p class="mt-2 text-gray-600 dark:text-gray-400">Update customer review information</p>
        </div>

        <form action="{{ route('admin.reviews.update', $review) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="review_text" 
                          ...
                          rows="4"
                          class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                          placeholder="Customer's review text...">{{ old('review_text', $review->review_text) }}</textarea>
                @error('review_text')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Update Image (Optional - Max 10MB)
                </label>
